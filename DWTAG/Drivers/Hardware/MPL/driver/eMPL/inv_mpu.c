 ...
 */
	...
	#include "sys.h"
//	#include "log.h"	
	#define i2c_write   I2C_WriteRegister_9250
	#define i2c_read    I2C_ReadRegister_9250
	#define get_ms      get_msticks
	#define log_i       printf
//	#define log_e       MPL_LOGE
	#define min(a,b) ((a<b)?a:b)
	
#elif defined EMPL_TARGET_STM32F0
	#include "I2C.h"
	#include "main.h"
//	#include "log.h"	
	#define i2c_write   I2C_WriteRegister_9250
...
int mpu_reset_fifo(void)
{
    ...
    if (st.chip_cfg.dmp_on) {
        ...
        data = BIT_FIFO_RST;
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))//reset fifo
            return -1;
        ...
        if (st.chip_cfg.int_enable)
            ...
            data = 0;
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))//data ready interrupt
            return -1;
...
static int gyro_6500_self_test(long *bias_regular, long *bias_st, int debug)
{
	...
	/* Self Test Pass/Fail Criteria C */
		...
		for (i = 0; i < 3; i++) {
			if(abs(bias_regular[i]) > gyro_offset_max) {
				if(debug)
...
//        log_e("Compass not found.\n");
    ...
    st.chip_cfg.mag_sens_adj[2] = (long)data[2] + 128;
		...
		pmag_sens_adj=st.chip_cfg.mag_sens_adj;
