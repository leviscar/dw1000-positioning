...
#define USART_RX_BUF usart_rx_buff
...
void usmart_reset_runtime(void)
{
  ...
  TIM14->ARR=0xffff;
  TIM14->CNT=0;
//	TIM14->EGR=0x0001;//SET UG bit to generate an update event
...
{
	uint8_t sta,len;  
	if(USART_STA.RC)//串口接收完成？
	{					   
		len=USART_STA.count;	//得到此次接收到的数据长度
		USART_RX_BUF[len-2]='\0';	//在末尾加入结束符. 
